...
from sequence_models.samplers import ApproxBatchSampler, SortishSampler
...
from src.evodiff.collaters import D3PMCollater, OAMaskCollater
from src.evodiff.losses import D3PMCELoss, D3PMLVBLoss, OAMaskedCrossEntropyLoss
from src.evodiff.model import ByteNetLMTime
from src.evodiff.utils import Tokenizer

sys.setrecursionlimit(
    1000
)  # must be as large as diffusion timesteps for Q_bar calculation
...
def main():
    ...
    parser.add_argument(
        "-off", "--offset", default=0, type=int, help="Number of GPU devices to skip."
    )
    ...
    parser.add_argument("--model_size", type=str, default="38M")
    parser.add_argument(
        "--aml", action="store_true"
    )  # Set true to do multi-node training on amlk8s
    parser.add_argument("-sd", "--state_dict", default=None)
    parser.add_argument("--decay", action="store_true")
    parser.add_argument("--final_norm", action="store_true")
    parser.add_argument(
        "--norm_first", action="store_true"
    )  # turns norm_first on in transformer model
    parser.add_argument(
        "--mini_run", action="store_true"
    )  # Set to True if running on subset of data
    parser.add_argument(
        "--mask", type=str, default="oadm"
    )  # Set to True if running on subset of data
    parser.add_argument(
    ...
    parser.add_argument(
        "--reweighting_term", type=float, default=0
    ...
    )  # ONLY USE THIS FLAG FOR FIRST RUN OF PRETRAIN
...
def train(gpu, args):
    ...
    # NOTE add the tensorboard
    now = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    writer = SummaryWriter(f"data/tensorboard/{now}{args.note}/")
    if args.aml:
    ...
    try:
        ...
        data_top_dir = home + "/data/DMs/"
        ptjob = False
    data_dir = data_top_dir + config["dataset"] + "/"
    if args.mini_run:
        mini_size = 100  # For troubleshooting
    # NOTE replace the author fpath setting
    if "dataset" in config:
        args.out_fpath = args.out_fpath + f"/{args.model_size}/{config['dataset']}/"
    else:
        args.out_fpath = args.out_fpath + f"/{args.model_size}/"
    # ----------------------------------------------------------
    ### COLLATORS ###
    # ----------------------------------------------------------
    if args.mask == "oadm":
    ...
    #     diffusion_timesteps = None  # Not input to model
        ...
        tokenizer = Tokenizer(
            path_to_blosum=data_top_dir + "blosum62-special-MSA.mat", sequences=True
        )
        if args.mask == "random":
        ...
        collater = D3PMCollater(
            tokenizer=tokenizer, num_timesteps=diffusion_timesteps, Q=Q_t, Q_bar=Q_prod
        )
    else:
    ...
    if args.mini_run:
        ...
        train_sortish_sampler = SortishSampler(
            len_train, bucket_size, num_replicas=args.world_size, rank=rank
        )
        train_sampler = ApproxBatchSampler(
            train_sortish_sampler, max_tokens, max_batch_size, len_train
        )
        dl_train = DataLoader(
            dataset=ds_train,
    ...
    if rank == 0:
        ...
        if args.mini_run:
            ...
            valid_sortish_sampler = SortishSampler(
                len_valid, 1000, num_replicas=1, rank=0
            )
            valid_sampler = ApproxBatchSampler(
                valid_sortish_sampler, max_tokens // 2, max_batch_size, len_valid
            )
            dl_valid = DataLoader(
                dataset=ds_valid,
    ...
    if args.pretrained:
        args.state_dict = f"data/model/preTrain/oaar-{args.model_size}.tar"
    ...
    def epoch(model, train, current_step=0, current_tokens=0):
        ...
        for i, batch in enumerate(loader):
            ...
            if (
                ...
                and i == 1
                and e == initial_epoch
                and args.state_dict is not None
                and not args.pretrained
            ):
                print("Restarting from checkpoint")
                optimizer.load_state_dict(sd["optimizer_state_dict"])
                scheduler.load_state_dict(sd["scheduler_state_dict"])
            new_loss, new_nll_loss, new_accu, new_n, new_seqs, new_processed = step(
                model, batch, train
            )
            if train:
            ...
            if rank == 0:
                ...
                print(
                    start
                    + "%s Epoch %d of %d Step %d ntokens %d Example %d of %d loss = %.4f nll loss = %.4f accu = %.4f"
                    % (
                        t,
                        e + 1,
                        epochs,
                ...
                # NOTE step tensorboard
                writer.add_scalar(f"{t}/step/loss", float(r_loss), nsteps)
                writer.add_scalar(f"{t}/step/nll loss", float(r_nll_loss), nsteps)
                writer.add_scalar(f"{t}/step/accu", float(raccu), nsteps)
            if train:
                losses = losses[-999:]
                accus = accus[-999:]
                ns = ns[-999:]
                ...
                if nsteps % args.log_freq == 0:  # write to checkpoint frequency
                    if rank == 0:
                        with open(args.out_fpath + "train-metrics.csv", "a") as f:
                            ...
                            f.write("\n")
                if (
                    (datetime.now() - chunk_time)
                    > timedelta(minutes=args.checkpoint_freq)
                ) or (n_seen == n_total):
                    if rank == 0:
    ...
    def step(model, batch, train):
        ...
        with torch.cuda.amp.autocast(dtype=torch.float32):
            ...
            if args.mask == "blosum" or args.mask == "random":
                ...
                ce_loss, nll_loss = loss_func(
                    outputs, tgt, mask, timestep, input_mask
                )  # sum(loss per token)
                loss = ce_loss
