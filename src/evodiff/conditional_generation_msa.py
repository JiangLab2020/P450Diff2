...
from src import evodiff
from src.evodiff.plot import aa_reconstruction_parity_plot, idr_parity_plot
from src.evodiff.pretrained import MSA_OA_DM_MAXSUB, MSA_OA_DM_RANDSUB, ESM_MSA_1b
from src.evodiff.utils import (
    Tokenizer,
    clean_pdb,
    read_dr_bert_output,
    run_omegafold,
...
def main():
    ...
    parser.add_argument(
        "--max-seq-len", type=int, default=150, help="Max seq len to splice from MSA"
    )
    parser.add_argument(
        "--n-sequences",
        type=int,
    ...
    if args.model_type == "msa_oa_dm_randsub":
        ...
        raise Exception(
            "Please select either msa_oa_dm_randsub, msa_oa_dm_maxsub, or esm_msa_1b baseline. You selected:",
            args.model_type,
        )
    ...
    if args.amlt:
        ...
        home = str(pathlib.Path.cwd()) + "/data/DMs/"
        top_dir = home
    ...
    if args.cond_task == "idr":
        if not os.path.exists(
            data_top_dir + "human_idr_alignments/human_idr_boundaries_gap.tsv"
        ):
        ...
        for i in range(args.num_seqs):
            ...
            string, og_string, new_idr, og_idr, start, end = generate_idr_msa(
                model,
                original_msa,
                src,
                num_sequences,
            ...
            start, end = ungap_index_IDR(
                start, end, string[0]
            )  # Reindex start/end for ungapped seq for dr_bert analysis
            ...
            b_string, b_og_string, b_new_idr, b_og_idr, b_start, b_end = (
                generate_idr_msa(
                    model,
                    original_msa,
                    b_src,
            ...
            if args.scramble_baseline:
                r_string, r_og_string, r_new_idr, r_og_idr, r_start, r_end = (
                    scramble_query(original_msa, start_idx, end_idx)
                )
                (
                    r_b_string,
                    r_b_og_string,
                    ...
                    r_b_start,
                    r_b_end,
                ) = scramble_query(original_msa, b_start_idx, b_end_idx)
        ...
        save_df = pd.DataFrame(
            list(zip(new_idrs, og_idrs, start_idxs, end_idxs)),
            columns=["gen_idrs", "original_idrs", "start_idxs", "end_idxs"],
        )
        save_df.to_csv(out_fpath + "idr_df.csv", index=True)
        og_save_df = pd.DataFrame(
            list(zip(og_idrs, og_idrs, og_start_idxs, og_end_idxs)),
            columns=["gen_idrs", "original_idrs", "start_idxs", "end_idxs"],
        )
        og_save_df.to_csv(out_fpath + "og_idr_df.csv", index=True)
        b_save_df = pd.DataFrame(
            list(zip(b_new_idrs, b_og_idrs, b_start_idxs, b_end_idxs)),
            columns=["gen_idrs", "original_idrs", "start_idxs", "end_idxs"],
        )
        b_save_df.to_csv(out_fpath + "og_baseline_idr_df.csv", index=True)
        og_b_save_df = pd.DataFrame(
            list(zip(new_idrs, og_idrs, og_b_start_idxs, og_b_end_idxs)),
            columns=["gen_idrs", "original_idrs", "start_idxs", "end_idxs"],
        )
        ...
        for i in range(args.num_seqs):  # no batching
            ...
            motif_end_idxs = [
                i + 1 for i in args.end_idxs
            ]  # inclusive of final residue
            # 50/50 split on MSAs
            if i < 50:
                selection_type = "random"
            else:
                selection_type = "MaxHamming"
            sliced_msa, sliced_start_idxs, sliced_end_idxs, original_motif = (
                subsample_MSA(
                    i,
                    data_top_dir,
                    args.pdb,
            ...
            string, new_start_idx, new_end_idx, seq_len = generate_scaffold_msa(
                args.model_type,
                model,
                sliced_msa,
                sliced_start_idxs,
        ...
        save_df = pd.DataFrame(
            list(zip(strings, start_idxs, end_idxs, scaffold_lengths)),
    ...
    if args.cond_task == "idr":
        ...
        wrap_dr_bert(
            out_fpath,
            generated_fasta_file="generated_samples_string.fasta",
            path_to_dr_bert=top_dir + "../DR-BERT/",
            out_file="gen_out.pkl",
        )
        wrap_dr_bert(
            out_fpath,
            generated_fasta_file="original_samples_string.fasta",
            path_to_dr_bert=top_dir + "../DR-BERT/",
            out_file="og_out.pkl",
        )
        wrap_dr_bert(
            out_fpath,
            generated_fasta_file="baseline_samples_string.fasta",
            path_to_dr_bert=top_dir + "../DR-BERT/",
            out_file="b_out.pkl",
        )
        wrap_dr_bert(
            out_fpath,
            generated_fasta_file="random_baseline_samples_string.fasta",
            path_to_dr_bert=top_dir + "../DR-BERT/",
            out_file="r_b_out.pkl",
        )
        wrap_dr_bert(
            out_fpath,
            generated_fasta_file="random_generated_samples_string.fasta",
            path_to_dr_bert=top_dir + "../DR-BERT/",
            out_file="r_gen_out.pkl",
        )

        true_disorder_score, true_order_score = evodiff.utils.read_dr_bert_output(
            out_fpath,
            "true",
        ...
        gen_disorder_score, gen_order_score = evodiff.utils.read_dr_bert_output(
            out_fpath,
            "gen",
        ...
        random_disorder_score, random_order_score = evodiff.utils.read_dr_bert_output(
            out_fpath,
            "random",
        ...
        plot_df = pd.DataFrame(
            {
                "score": true_disorder_score,
                "region": ["disorder"] * len(true_disorder_score),
        ...
        plot_df = plot_df.append(
            pd.DataFrame(
                {
                    "score": true_order_score,
                    "region": ["order"] * len(true_order_score),
                    "type": ["true"] * len(true_order_score),
                }
            ),
            ignore_index=True,
        )
        ...
        plot_df = plot_df.append(
            pd.DataFrame(
                {
                    "score": gen_disorder_score,
                    "region": ["disorder"] * len(gen_disorder_score),
                    "type": ["gen"] * len(gen_disorder_score),
                }
            ),
            ignore_index=True,
        )
        plot_df = plot_df.append(
            pd.DataFrame(
                {
                    "score": gen_order_score,
                    "region": ["order"] * len(gen_order_score),
                    "type": ["gen"] * len(gen_order_score),
                }
            ),
            ignore_index=True,
        )
        plot_df = plot_df.append(
            pd.DataFrame(
                {
                    "score": random_disorder_score,
                    "region": ["disorder"] * len(random_disorder_score),
                    "type": ["random"] * len(random_disorder_score),
                }
            ),
            ignore_index=True,
        )
        plot_df = plot_df.append(
            pd.DataFrame(
                {
                    "score": random_order_score,
                    "region": ["order"] * len(random_order_score),
                    "type": ["random"] * len(random_order_score),
                }
            ),
            ignore_index=True,
        )
        ...
        evodiff.plot.idr_boxplot_all(
            plot_df, out_fpath + "plots/", save_name="combined_"
        )
        evodiff.plot.idr_boxplot(
            true_disorder_score,
            true_order_score,
            out_fpath + "plots/",
            save_name="true_",
        ...
        evodiff.plot.idr_boxplot(
            gen_disorder_score, gen_order_score, out_fpath + "plots/", save_name="gen_"
        )
        evodiff.plot.idr_boxplot(
            random_disorder_score,
            random_order_score,
            out_fpath + "plots/",
            save_name="random_",
        )
        ...
        run_tmscore(
            ...
            args.pdb,
            args.num_seqs,
            path_to_tmscore=top_dir + "TMscore",
            amlt=args.amlt,
        )
...
def subsample_MSA(
    ...
    tokenized_msa = get_MSA(
        data_top_dir + "/scaffolding-msas/" + pdb + ".a3m", tokenizer=Tokenizer()
    )
    original_motif = [
        tokenizer.untokenize(tokenized_msa[0][start_idxs[i] : end_idxs[i]])
    ...
    if os.path.isfile(msa_save_file):
        ...
        if msa_seq_len > max_seq_len:
            ...
            if motif_start - buffer < 0:  # if MOTIF at beginning of seq
                ...
                sliced_start_idxs = [
                    start_idx - slice_start for start_idx in start_idxs
                ]
            else:  # center IDR
                print("CENTER")
        ...
        sliced_msa_seq = np.append(
            sliced_msa_seq[:query_idx], sliced_msa_seq[query_idx + 1 :], axis=0
        )
        # Query Sequence
        anchor_seq = tokenized_msa[
            query_idx, slice_start:slice_end
        ]  # This is the query sequence
        sliced_msa = [
            seq for seq in sliced_msa_seq if (list(set(seq)) != [tokenizer.gap_id])
        ]
        ...
        if msa_num_seqs > n_sequences:
            ...
            output = np.full(
                shape=(n_sequences, max_seq_len), fill_value=tokenizer.gap_id
            )  # Treat short seqs as being algined with large gaps
        ...
        output_motif = [
            tokenizer.untokenize(output[0][sliced_start_idxs[i] : sliced_end_idxs[i]])
            for i in range(len(sliced_start_idxs))
        ]
        print("og", original_motif)
        print("out", output_motif)
        assert original_motif == output_motif, (
            "RE-SLICED MOTIFS DON'T MATCH, CHECK INDEXING"
        )
        output = [tokenizer.untokenize(seq) for seq in output]
...
def get_masked_locations(query_sequence, sliced_start_idxs, sliced_end_idxs, pad_id):
    ...
    list_motif = [
        list(range(sliced_start_idxs[i], sliced_end_idxs[i]))
        for i in range(len(sliced_start_idxs))
    ]
...
def tokenize_msa(model_type, untokenized, tokenizer):
    if model_type == "msa_oa_dm_maxsub" or model_type == "msa_oa_dm_randsub":
        ...
        for i, seq in enumerate(untokenized):
            new_seq = (
                [tokenizer.cls_idx]
                + [tokenizer.get_idx(c) for c in [*seq]]
                + [tokenizer.eos_idx]
            )
            src.append(new_seq)
        return src
...
def generate_scaffold_msa(
    ...
    if random_baseline:
        train_prob_dist = aa_reconstruction_parity_plot(
            data_top_dir + "../", "reference/", "placeholder.csv", gen_file=False
        )
    ...
    if model_type == "esm_msa_1b":
        ...
        mask_locations = get_masked_locations(
            query_sequence[1:-1], sliced_start_idxs, sliced_end_idxs, pad_id=pad
        )
        ...
        seq_len = len(query_sequence)
        mask_locations = get_masked_locations(
            query_sequence, sliced_start_idxs, sliced_end_idxs, pad_id=pad
        )
    ...
    masked_loc_x = [0]
    query_ind = np.transpose(
        [
            np.tile(masked_loc_x, len(masked_loc_y)),
            np.repeat(masked_loc_y, len(masked_loc_x)),
        ]
    )
        ...
        if not query_only:
            ...
            for i in tqdm(all_ind):
                ...
                if model_type == "esm_msa_1b":
                    ...
                    preds = model(
                        sample
                    )  # Output shape of preds is (BS=1, N=64, L, n_tokens=31)
                p = preds[
                    :, random_x, random_y, :
                ]  # for first row don't let p_softmax predict gaps
    ...
    untokenized = [
        untokenize_msa(model_type, sample[0][0], tokenizer)
    ]  # only return query sequence
    print(untokenized)

    return (
        untokenized,
        sliced_start_idxs,
        [i - 1 for i in sliced_end_idxs],
        seq_len,
    )  # return output and untokenized output, re-indexed motif starts and ends (ends-1 for rmsd analyis)
...
def scramble_query(original_msa, start_idx, end_idx):
    ...
    scrambled_sequence = [
        original_msa[0][:start_idx] + scrambled_idr + original_msa[0][end_idx:]
    ]
    # print("full sequence", scrambled_sequence)
    print(len(scrambled_sequence[0]), len(original_msa[0]))
    assert len(scrambled_sequence[0]) == len(original_msa[0]), (
        "SCRAMBLED seq different length"
    )
...
def generate_idr_msa(
    ...
    with torch.no_grad():
        for i in tqdm(all_ind):
            ...
            random_x, random_y = i
            if random_baseline:
                p_sample = torch.multinomial(
                    torch.tensor(train_prob_dist), num_samples=1
                )
            else:
                # print(sample.shape)
                preds = model(
    ...
    # import pdb; pdb.set_trace()
    return (
        untokenized_query_msa,
        original_msa[0],
        new_idr,
        og_idr,
        start_idx,
        end_idx,
    )  # return gen_query, og_query, new_idrs, og_idrs
...
def reindex_IDR(start_idx, end_idx, query_seq, gapped_query_seq):
    ...
    """
    old_idx = list(
        np.arange(1, len(query_seq) + 1)
    )  # This starts at 1 and is inclusive
    gap_count = 0
...
def preprocess_IDR_data(data_top_dir):
    ...
    for index, row in index_file.iterrows():
        msa_file = [file for i, file in enumerate(all_files) if row["OMA_ID"] in file][
            0
        ]
        msa_data, msa_names = parse_fasta(
            data_dir + "human_protein_alignments/" + msa_file, return_names=True
        )
        query_idx = [i for i, name in enumerate(msa_names) if name == row["OMA_ID"]][
            0
        ]  # get query index
        seq_only = msa_data[query_idx].replace("-", "")
        start_idx = row["START"]
        end_idx = row["END"]
        new_start_idx, new_end_idx = reindex_IDR(
            start_idx, end_idx, seq_only, msa_data[query_idx]
        )
...
def get_IDR_MSAs(
    ...
    if not query_only:
        masked_msa = [
            mask_idr(seq, new_start_idx, new_end_idx, i, num_sequences)
            for i, seq in enumerate(msa_data)
        ]
    ...
    if not query_only:
        b_masked_msa = [
            mask_idr(seq, b_start_idx, b_end_idx, i, num_sequences)
            for i, seq in enumerate(msa_data)
        ]
...
def subsample_IDR_MSA(
    ...
    for i in range(len(index_file)):
        while count < 1:
            ...
            # Get MSA
            msa_file = [
                file for i, file in enumerate(all_files) if row["OMA_ID"] in file
            ][0]
            parsed_msa, msa_names = parse_fasta(
                data_dir + "human_protein_alignments/" + msa_file, return_names=True
            )
            # Get query
            query_idx = [
                i for i, name in enumerate(msa_names) if name == row["OMA_ID"]
            ][0]  # get query index
            ...
            for i in range(len(query_rows)):
                idr_range = np.arange(
                    query_rows.iloc[i]["GAP START"], query_rows.iloc[i]["GAP END"]
                )
            ...
            if (
                msa_seq_len > max_seq_len
                and idr_len < (max_seq_len / 2)
                and len(non_idr_ranges) > 0
            ):
                ...
                if new_start_idx - buffer < 0:  # if IDR at beginning of seq
                    ...
                    if (
                        b_start_idx + idr_len_nogaps >= max_seq_len
                    ):  # Reduce baseline length if longer than max len
                ...
                ):  # If slicing sequence length 511 in center, correct length
                    slice_end += max_seq_len - (slice_end - slice_start)
                    print("ADJUST SLICING INDEX", slice_start, slice_end)
                print(
                    "IDR INDEX, adjust slice", sliced_idr_start_idx, sliced_idr_end_idx
                )
                # seq_len = max_seq_len
                count += 1
            elif (
                msa_seq_len < max_seq_len
                and idr_len < msa_seq_len / 2
                and len(non_idr_ranges) > 0
            ):
                print("SHORT SEQ")
                slice_start = 0
    ...
    print(
        "ANCHOR SEQ",
        len(tokenizer.untokenize(anchor_seq)),
        tokenizer.untokenize(anchor_seq),
    )
    ...
    print(
        ...
        tokenizer.untokenize(
            anchor_seq[sliced_idr_start_idx:sliced_idr_end_idx]
        ).replace("-", ""),
    )
    print(
        "BASELINE SEQ",
        tokenizer.untokenize(anchor_seq[b_start_idx:b_end_idx]).replace("-", ""),
    )
    ...
    if msa_num_seqs > n_sequences:
        ...
        if selection_type == "random":
            print("Using random subsampling")
            random_idx = np.random.choice(
                msa_num_seqs, size=n_sequences - 1, replace=False
            )
            anchor_seq = np.expand_dims(anchor_seq, axis=0)
            output = np.concatenate(
                (anchor_seq, np.array(sliced_msa)[random_idx.astype(int)]), axis=0
