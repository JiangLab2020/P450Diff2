import numpy as np
import torch
from sequence_models.constants import GAP, PAD, PROTEIN_ALPHABET
...
def _pad_msa(tokenized, num_seq, max_len, value, dim=3):
    ...
    if dim == 4:  # one hot MSA
        ...
        output = (
            torch.zeros((batch_size, num_seq, max_len, categories), dtype=torch.long)
            + value
        )
...
class OAMaskCollater(object):
    ...
    def __call__(self, sequences):
        ...
        for i, x in enumerate(tokenized):
            ...
            # Generate mask
            mask_arr = np.random.choice(
                D, num_mask, replace=False
            )  # Generates array of len num_mask
            index_arr = np.arange(0, max_len)  # index array [1...seq_len]
            mask = np.isin(index_arr, mask_arr, invert=False).reshape(
                index_arr.shape
            )  # mask bools indices specified by mask_arr
            # Mask inputs
...
class D3PMCollater(object):
    ...
    def __init__(self, tokenizer=Tokenizer(), num_timesteps=100, Q=None, Q_bar=None):
        self.tokenizer = tokenizer
        self.num_timesteps = (
            num_timesteps  # Only needed for markov trans, doesnt depend on seq len
        )
    ...
    def __call__(self, sequences):
        ...
        for i, t in enumerate(tokenized):  # enumerate over batch
            ...
            x_t, q_x_t = sample_transition_matrix(
                x, self.Q_bar[t]
            )  # x = tgt, x_t = src, Q_bar[t] is cum prod @ time t
            src.append(x_t)
        ...
        return (
            src.to(torch.long),
            src_one_hot.to(torch.double),
            torch.tensor(timesteps),
            tokenized.to(torch.long),
            one_hot.to(torch.double),
            self.Q,
            self.Q_bar,
            q_x.to(torch.double),
        )  # , q_x_minus1.to(torch.double))
...
class D3PMCollaterMSA(object):
    ...
    def __init__(
        self, tokenizer=Tokenizer(), num_timesteps=100, Q=None, Q_bar=None, num_seqs=64
    ):
        self.tokenizer = tokenizer
        self.num_timesteps = (
            num_timesteps  # Only needed for markov trans, doesnt depend on seq len
    ...
    def __call__(self, msas):
        ...
        max_seq_len = max(
            len(t[0]) for t in tokenized
        )  # all seqs in MSA are the same len
        timesteps = []
        src = tokenized.copy()
        ...
        for i in range(batch_size):  # enumerate over batch
            tokenized[i] = [
                torch.tensor(self.tokenizer.tokenizeMSA(s)) for s in msas[i]
            ]
            tgt_one_hot[i] = [self.tokenizer.one_hot(t) for t in tokenized[i]]
            curr_msa = torch.stack(tgt_one_hot[i])
            length, depth, tokens = (
                curr_msa.shape
            )  # length = number of seqs in MSA, depth = # AA in MSA
            ...
            # Calculate target
            x_t, q_x_t = sample_transition_matrix(
                curr_msa, self.Q_bar[t]
            )  # x = tgt, x_t = src, Q_bar accounts for time
            # Reshape back to MSA
        ...
        tokenized = _pad_msa(
            tokenized, self.num_seqs, max_seq_len, self.tokenizer.pad_id
        )
        src_one_hot = _pad_msa(
            src_one_hot, self.num_seqs, max_seq_len, self.tokenizer.pad_id, dim=4
        )
        tgt_one_hot = _pad_msa(
            tgt_one_hot, self.num_seqs, max_seq_len, self.tokenizer.pad_id, dim=4
        )
        return (
            src.to(torch.long),
            src_one_hot.to(torch.double),
...
class ESMOAMaskCollater(object):
    ...
    def __call__(self, sequences):
        ...
        for i, x in enumerate(sample):
            ...
            D = len(
                x
            )  # D should have the same dimensions as each sequence length, minus start stop token
            t = np.random.randint(
                1, D - 1
            )  # randomly sample timestep (don't want to sample start/stop token)
            num_mask = D - t + 1  # from OA-ARMS
            # Append timestep
            timesteps.append(num_mask)
            # Generate mask
...
class ESMOAMaskCollaterMSA(object):
    ...
    def __call__(self, msa_batch):
        ...
        tgt = [
            [[self.alphabet.get_idx(c) for c in [*seq]] for seq in msa]
            for msa in batch_strs
        ]
        # tgt = sample
        # print(sample[0][0])
        # print(tgt[0][0])
        ...
        src = torch.stack(
            src
        )  # No batches #_pad_msa(src, self.num_seqs, self.alphabet.padding_idx, longest_msa)
