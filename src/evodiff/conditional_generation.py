...
from src.evodiff.plot import aa_reconstruction_parity_plot
from src.evodiff.pretrained import (
    CARP_640M,
    LR_AR_38M,
    LR_AR_640M,
    OA_DM_38M,
...
from src.evodiff.utils import Tokenizer, clean_pdb, run_omegafold, run_tmscore
...
def main():
    ...
    parser.add_argument("--chain", type=str, default="A", help="chain in PDB")
    parser.add_argument(
        "--scaffold-min", type=int, default=100, help="Min scaffold len "
    )
    parser.add_argument(
        "--scaffold-max",
        type=int,
    ...
    )  # either random-baseline or scrambled baseline NOT BOTH
    ...
    if args.amlt:
        ...
        home = str(pathlib.Path.cwd()) + "/data/evodiff/"
        top_dir = home
    ...
    data_top_dir = "/home/v-salamdari/data/DMs/data/"
    ...
    if args.cond_task == "idr":
        ...
        if args.scrambled_baseline:
            r_strings, r_og_strings, r_new_idrs, r_og_idrs, r_start_idxs, r_end_idxs = (
                scramble_input(sequences, start_idxs, end_idxs)
            )
            (
                r_b_strings,
                r_b_og_strings,
                ...
                r_b_start_idxs,
                r_b_end_idxs,
            ) = scramble_input(sequences, b_starts, b_ends)
        else:
            r_strings, r_og_strings, r_new_idrs, r_og_idrs, r_start_idxs, r_end_idxs = (
                inpaint(
                    model,
        ...
        strings, og_strings, new_idrs, og_idrs, start_idxs, end_idxs = inpaint(
            model,
            tokenized_sequences,
            start_idxs,
            end_idxs,
        ...
        b_strings, b_og_strings, b_new_idrs, b_og_idrs, b_start_idxs, b_end_idxs = (
            inpaint(
                model,
                b_tokenized,
                b_starts,
        ...
        r_save_df = pd.DataFrame(
            list(zip(r_new_idrs, r_og_idrs, r_start_idxs, r_end_idxs)),
            columns=["gen_idrs", "original_idrs", "start_idxs", "end_idxs"],
        )
        ...
        r_b_save_df = pd.DataFrame(
            list(zip(r_b_new_idrs, r_b_og_idrs, r_b_start_idxs, r_b_end_idxs)),
            columns=["gen_idrs", "original_idrs", "start_idxs", "end_idxs"],
        )
        ...
        for i in range(args.num_seqs):
            ...
            if (
                args.model_type == "oa_dm_38M"
                or args.model_type == "oa_dm_640M"
                or args.model_type == "carp_38M"
                or args.model_type == "carp_640M"
            ):
                string, new_start_idx, new_end_idx = generate_scaffold(
                    model,
                    args.pdb,
                    args.start_idxs,
                    args.end_idxs,
                ...
                string, new_start_idx, new_end_idx = generate_autoreg_scaffold(
                    model,
                    args.pdb,
                    args.start_idxs,
                    args.end_idxs,
    ...
    if args.cond_task == "idr":
        ...
        #     evodiff.utils.wrap_disopred(fasta_file=out_fpath+'fasta/gen_seq_'+str(i)+'.fasta', path_to_disopred='/home/v-salamdari/data/disopred/BLAST+/run_disopred_plus.pl')
        #     evodiff.utils.wrap_disopred(fasta_file=out_fpath+'fasta/true_seq_'+str(i)+'.fasta', path_to_disopred='/home/v-salamdari/data/disopred/BLAST+/run_disopred_plus.pl')
        #     evodiff.utils.wrap_disopred(fasta_file=out_fpath + 'fasta/base_gen_seq_' + str(i) + '.fasta', path_to_disopred='/home/v-salamdari/data/disopred/BLAST+/run_disopred_plus.pl')
        #     evodiff.utils.wrap_disopred(fasta_file=out_fpath + 'fasta/r_base_gen_seq_' + str(i) + '.fasta', path_to_disopred='/home/v-salamdari/data/disopred/BLAST+/run_disopred_plus.pl')
        #     evodiff.utils.wrap_disopred(fasta_file=out_fpath+'fasta/r_gen_seq_'+str(i)+'.fasta', path_to_disopred='/home/v-salamdari/data/disopred/BLAST+/run_disopred_plus.pl')
...
def get_motif(PDB_ID, start_idxs, end_idxs, data_top_dir="../data", chain="A"):
    ...
    pdb_clean_path = os.path.join(
        data_top_dir, "scaffolding-pdbs/" + str(PDB_ID) + "_clean.pdb"
    )
...
def generate_scaffold(
    ...
    motif_seq = get_motif(
        PDB_ID, motif_start_idxs, motif_end_idxs, data_top_dir=data_top_dir, chain=chain
    )
    ...
    value, loc = (
        (sample == mask).long().nonzero(as_tuple=True)
    )  # locations that need to be unmasked
    loc = np.array(loc)
    ...
    with torch.no_grad():
        for i in loc:
            timestep = torch.tensor(
                [0] * batch_size
            )  # placeholder but not called in model
            ...
            if random_baseline:
                ...
                p = torch.nn.functional.softmax(
                    p, dim=1
                )  # softmax over categorical probs
...
def generate_autoreg_scaffold(
    ...
    new_start = np.random.choice(scaffold_length)  # randomly place motif in scaffold
    sample_ref[:, new_start : new_start + len(motif_seq)] = torch.tensor(
        motif_tokenized
    )
    ...
    print(new_start_idxs, new_end_idxs)
    value, loc = (
        (sample_ref == mask).long().nonzero(as_tuple=True)
    )  # locations that need to be unmasked
    loc = np.array(loc)
    ...
    with torch.no_grad():
        ...
        while i < max_seq_len:
            ...
            if (
                i > new_end_idxs[-1]
            ):  # Force it to continue predicting until you reach the end of motif
            ...
            if reach_stop == False:
                if i in new_start_idxs:  # if index is a new start idx
                    ...
                    sliced_motif = sample_ref[
                        :, new_start_idxs[i_index] : new_end_idxs[i_index] + 1
                    ]
                    ...
                    prediction = model(
                        sample, timestep
                    )  # , input_mask=input_mask.unsqueeze(-1)) #sample prediction given input
                ...
                break
    print(
        "new sequence", [tokenizer.untokenize(s) for s in sample[:, 1:-1]]
    )  # dont need start/stop tokens
...
def scramble_input(sequences, start_idxs, end_idxs):
    ...
    for s, sequence in enumerate(sequences):
        ...
        scrambled_sequence = (
            sequence[: start_idxs[s]] + scrambled_idr + sequence[end_idxs[s] :]
        )
        # print("full sequence", scrambled_sequence)
        assert len(scrambled_sequence) == len(sequence), (
...
def inpaint_simple(
    model, sequence, start_idx, end_idx, tokenizer=Tokenizer(), device="cuda"
):
...
def inpaint(
    ...
    for s, sample in enumerate(tokenized_sequences):
        ...
        print(
            s,
            "GENERATED REGION",
            tokenizer.untokenize(sample[start_idxs[s] : end_idxs[s]]),
        )
    ...
    return (
        ...
        save_starts,
        save_ends,
    )  # strings, og_strings, new_idrs, og_idrs
    ...
    while appended_seqs < num_seqs:
        for _ in range(len(index_file)):
            rand_idx = random.randint(
                0, len(index_file) - 1
            )  # Iterate over all or randomly select
            ...
            for i in range(len(query_rows)):
                idr_range = np.arange(
                    query_rows.iloc[i]["START"], query_rows.iloc[i]["END"]
                )
            ...
            if (
                ...
                masked_sequence = (
                    seq_only[0:start_idx] + "#" * idr_length + seq_only[end_idx:]
                )
                ...
                b_masked_sequence = (
                    seq_only[0:b_start] + "#" * idr_length + seq_only[b_end:]
                )
