...
class OAMaskedCrossEntropyLoss(CrossEntropyLoss):
    ...
    def __init__(
        self, weight=None, reduction="none", reweight=True, tokenizer=Tokenizer()
    ):
    ...
    def forward(self, pred, tgt, mask, timesteps, input_mask):
        ...
        p = torch.masked_select(pred, mask).view(
            mask_tokens, -1
        )  # [T x K] predictions for each mask char
...
class D3PMCELoss(CrossEntropyLoss):
    ...
    def __init__(
        self, weight=None, reduction="mean", tokenizer=Tokenizer(), sequences=True
    ):
        self.tokenizer = tokenizer
...
class D3PMLVBLoss(KLDivLoss):
    ...
    def __init__(
        self, tmax=500, reduction="batchmean", log_target=False, tokenizer=Tokenizer()
    ):
        self.tmax = tmax
        self.tokenizer = tokenizer
    ...
    def forward(
        ...
        for i in range(tgt.shape[0]):  # enumerate over batch
            D = int(
                nonpad_loc[i].item()
            )  # want prior/q in shape of seq len (q has shape of longest seq in batch)
            if timestep[i] == 1:
                # CE (L_t=0)
                ...
                kl_loss_i = super().forward(
                    prior.log(), q_true
                )  # fKLDivLoss expects input in log-space
                ...
                Q_expand = (
                    Q_bar[timestep[i] - 1]
                    .unsqueeze(0)
                    .expand(A.shape[0], self.K, self.K)
                )  # [ P x K x K]
                B_pred = torch.mul(pred.unsqueeze(2), Q_expand)

                q_t = torch.mul(A.unsqueeze(1), B_pred)
                p_theta_marg = torch.bmm(
                    torch.transpose(q_t, 1, 2), pred.unsqueeze(2)
                ).squeeze()  # mul and sum over model logits

                # calculate q_t_minus_1, simplified for loops
                num = torch.mul(A, B)
                denom = torch.bmm(
                    torch.mm(x_0, Q_bar[timestep[i]]).unsqueeze(1), x_t.unsqueeze(2)
                )
                q_t_minus1 = num / denom.squeeze().unsqueeze(1)

                p_theta_marg = p_theta_marg / p_theta_marg.sum(
                    axis=1, keepdim=True
                )  # re-normalize probs per residue
                p_theta_marg = p_theta_marg.to(tgt.device)
                kl_loss_i = super().forward(
                    p_theta_marg.log(), q_t_minus1
...
class D3PMLVBLossMSA(KLDivLoss):
    ...
    def forward(
        ...
        for i in range(len(tgt)):  # enumerate over batch
            D = int(
                nonpad_loc[i][0]
            )  # all seq in one MSA are padded to the same length, use first seq as ref
            if timestep[i] == 1:
                # CE (L_t=0)
                r_loss = self.reconstruction_loss(
                    predictions[i].unsqueeze(0),
                ...
                # As T approches infinity, this term goes to zero
                q_true = q[i, :, :D, :]
                prior = sample_priorMSA(
                    q_true.shape[0],
                    q_true.shape[1],
                    q_true.shape[2],
                    _len=self.tokenizer.alphabet,
                )
                prior = prior.to(tgt.device)
                kl_loss_i = super().forward(
                    prior.log(), q_true
                ...
                B_pred = torch.mul(pred.unsqueeze(2), Q_expand)
                q_t = torch.mul(
                    A.unsqueeze(1), B_pred
                )  # batch mul (broadcast logit dim)
                p_theta_marg = torch.bmm(
                    torch.transpose(q_t, 1, 2), pred.unsqueeze(2)
                ).squeeze()  # this marginalizes over logits
                p_theta_marg = p_theta_marg / p_theta_marg.sum(
                    axis=1, keepdim=True
                )  # renormalize probabilities at each position before KL
                p_theta_marg = p_theta_marg.to(tgt.device)

                # calculate q_t_minus_1, simplified for loops
                num = torch.mul(A, B)
                denom = torch.bmm(
                    torch.mm(x_0, Q_bar[timestep[i] - 1]).unsqueeze(1), x_t.unsqueeze(2)
                )
                q_t_minus1 = num / denom.squeeze().unsqueeze(1)
