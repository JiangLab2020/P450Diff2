...
def aa_reconstruction_parity_plot(
    project_dir,
    out_path,
    generate_file,
    msa=False,
    ...
    if msa:
        ...
        file = project_dir + "ref/uniref50_aa_ref_test.csv"
        # print(file)
    if idr:
        idr_flag = "idr_"
    ...
    if start_valid:
        ...
        # print(aminos)
        a_kl = normalize_list(
            [each[0] for each in removekey(aminos, keys_to_remove).values()]
        )
    if gen_file:
        ...
        print("KL", kl)
        with open(out_path + idr_flag + f"generate_metrics_{fileName}.csv", "w") as f:
        ...
        plt.tight_layout()
        fig.savefig(os.path.join(out_path, idr_flag + f"parity_scatter_{fileName}.svg"))
        fig.savefig(os.path.join(out_path, idr_flag + f"parity_scatter_{fileName}.png"))
        plt.close()
    if not gen_file:
...
def msa_substitution_rate(generated_msa, train_msa, alphabet, out_path):
    ...
    train_table, train_vals, train_diag_vals = normalize_matrix(
        train_matrix.T, alphabet
    )
    ...
    for i, ax in enumerate(axes.ravel()[: len(alphabet)]):
        ...
        ax.scatter(train_vals[i, :], gen_vals[i, :], color="blue", linewidth=0, alpha=1)
        ax.scatter(train_vals[i, i], gen_vals[i, i], color="red", linewidth=0, alpha=1)
    ...
    ax.text(
        0.05, 0.95, label, transform=ax.transAxes, fontsize=14, verticalalignment="top"
    )
    plt.tight_layout()
...
def msa_pairwise_interactions(
    generated_msa, train_msa, all_aa, out_path
):  # Look at AA pairwise interactions within each MSA within each sample
    ...
    plt.scatter(
        train_vals, gen_vals, color="blue", linewidth=0, alpha=0.5
    )  # marker = alpha
...
def plot_ecdf_bylength(
    perp_groups, colors, labels, seq_lengths, metric="perp", model="esm-if"
):
    "Plots cumulative density as a function of sequence length"
    fig, ax = plt.subplots(1, 4, figsize=(8.0, 2.5), sharey=True, sharex=True)
    for j, perp_group in enumerate(perp_groups):
        for i, p in enumerate(perp_group):
            c = colors[j]
            sns.ecdfplot(x=p, label=labels[j], color=c, alpha=1, ax=ax[i])
            if metric == "perp":
...
def plot_sc_boxplot(
    ...
    for i, perp_group in enumerate(perp_groups):
        ...
        [
            all_names.append(labels[i])
            for _ in range(len(list(chain.from_iterable(perp_group))))
        ]
    ...
    ax.axhline(
        y=np.median(list(chain.from_iterable(perp_groups[0]))), c="k", ls="--", lw=0.75
    )
    ...
    plt.tight_layout()
    fig.savefig(
        os.path.join("plots/sc_" + metric + "_" + model + "_" + length_model + ".svg")
    )
    fig.savefig(
        os.path.join("plots/sc_" + metric + "_" + model + "_" + length_model + ".png")
    )
...
def plot_ecdf(
    ...
    for i, perp_group in enumerate(perp_groups):
        ...
        sns.ecdfplot(x=all_perp, label=labels[i], color=c, alpha=1, ax=ax)
        if metric == "perp":
        ...
        ax.axvline(
            x=np.mean(list(chain.from_iterable(perp_groups[0]))),
            c="k",
            ls="--",
            lw=0.75,
        )
...
def plot_plddt_perp(
    ...
    plt.scatter(
        ordered_plddt_group[0],
        ordered_perp_group[0],
        c=colors[0],
        s=20,
        alpha=1,
        label=labels[0],
    ...
    plt.scatter(
        ordered_plddt_group[idx],
        ordered_perp_group[idx],
        c=colors[idx],
        s=20,
        alpha=1,
        label=labels[idx],
    ...
    plt.tight_layout()
    fig.savefig(
        os.path.join("plots/sc_plddt_perp_" + labels[idx] + "_" + length_model + ".svg")
    )
    fig.savefig(
        os.path.join("plots/sc_plddt_perp_" + labels[idx] + "_" + length_model + ".png")
    )
...
def plot_embedding(train_emb, run_emb, colors, i, runs, project_run):
    ...
    # Plot run
    plt.scatter(
        run_emb[:, 0], run_emb[:, 1], s=20, alpha=0.95, c=colors[i + 1], edgecolors="k"
    )
    ax.axis("off")
...
def plot_conditional_rmsd(pdb, motif_df, out_path="plots/"):
    ...
    ax[0].scatter(
        motif_df["scaffold_lengths"], motif_df["rmsd"], edgecolors="grey", c="#D0D0D0"
    )
    ...
    ax[2].scatter(
        motif_df["scores_fixed"], motif_df["rmsd"], edgecolors="grey", c="#527d99"
    )
...
def plot_idr_drbert_multiple(
    out_fpath, prefix, df, start, end, df2, start2, end2, save_iter
):
