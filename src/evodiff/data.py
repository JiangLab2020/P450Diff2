...
from sequence_models.constants import GAP, PAD, PROTEIN_ALPHABET, trR_ALPHABET
...
def subsample_msa(
    path_to_msa, n_sequences=64, max_seq_len=512, selection_type="random"
):
    ...
    if msa_num_seqs < n_sequences:
        ...
        raise Exception(
            "msa num_seqs < self.n_sequences, indicates dataset not filtered properly"
        )
    elif msa_num_seqs > n_sequences:
        if selection_type == "random":
            random_idx = (
                np.random.choice(msa_num_seqs - 1, size=n_sequences - 1, replace=False)
                + 1
            )
...
def get_msa_depth_lengths(
    data_dir, all_files, save_depth_file, save_length_file, idr=False
):
    """
...
def get_valid_msas(
    ...
    dataset = A3MMSADataset(
        ...
        n_sequences,
        max_seq_len,
        data_dir=os.path.join(data_top_dir, data_dir),
        min_depth=64,
    )
...
def get_idr_query_index(data_dir, all_files, save_file):
    ...
    for filename in tqdm(all_files):
        ...
        query_idx = [
            i for i, name in enumerate(msa_names) if name == filename.split("_")[0]
        ][0]  # get query index
...
def get_sliced_gap_depth_openfold(data_dir, all_files, save_file, max_seq_len=512):
    ...
    for filename in tqdm(all_files):
        ...
        parsed_msa = parse_fasta(path)
        sliced_msa_depth = [
            seq for seq in parsed_msa if (Counter(seq)[GAP]) <= max_seq_len
        ]  # Only append seqs with gaps<512
...
class TRRMSADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sliced_msa = [
            ...
            if (list(set(seq)) != [self.tokenizer.alphabet.index(GAP)])
        ]
...
class A3MMSADataset(Dataset):
    ...
    def __init__(
        self, selection_type, n_sequences, max_seq_len, data_dir=None, min_depth=None
    ):
        """
        Args:
            selection_type: str,
    ...
    def __getitem__(self, idx):
        ...
        # slice out all-gap rows
        sliced_msa = [
            seq for seq in sliced_msa_seq if (list(set(seq)) != [self.gap_idx])
        ]
        msa_num_seqs = len(sliced_msa)
...
class IDRDataset(Dataset):
    ...
    def __init__(
        ...
        if not os.path.exists(data_dir + "idr_lengths.npz"):
            raise Exception(
                "Missing idr_lengths.npz in human_idr_alignments/human_protein_alignments/"
            )
        if not os.path.exists(data_dir + "idr_depths.npz"):
            # get_msa_depth_openfold(data_dir, sorted(all_files), 'openfold_depths.npz')
            raise Exception(
                "Missing idr_depths.npz in human_idr_alignments/human_protein_alignments/"
            )
