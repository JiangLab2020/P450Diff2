...
from src.evodiff.constants import BLOSUM_ALPHABET
...
def loadMatrix(path):
    ...
    if not len(blosumDict) == len(labelslist) ** 2:
        ...
        raise EOFError(
            "Blosum file is not quadratic.", len(blosumDict), len(labelslist) ** 2
        )
    return blosumDict
...
def double_stochastic(q):
    ...
    while not np.isclose(
        np.min(np.sum(q_norm, axis=0)), 1
    ):  # only checking that one value converges to 1 (prob best to do all 4 min/max)
...
def _beta_schedule(num_timesteps, schedule="linear", start=1e-5, end=0.999, max=8):
    ...
    if schedule == "linear":
        ...
        print(
            "Must select a valid schedule; ['linear', 'sohl-dickstein', 'cosine', 'exp']"
        )
    return betas
...
def read_fasta(fasta_path, seq_file, info_file, index_file):
    ...
    """
    with (
        open(fasta_path) as f_in,
        open(seq_file, "w") as f_out,
        open(info_file, "w") as info_out,
        open(index_file, "w") as i_out,
    ):
...
def download_model(model_name):
    if model_name == "carp-38M":
        ...
        url = (
            f"https://zenodo.org/record/8045076/files/" + model_name + ".tar?download=1"
        )
    try:
...
def run_omegafold(fpath, fasta_file="generated_samples_string.fasta", gpu=0):
    ...
    if not os.path.exists(out_fpath):
        ...
        subprocess.run(
            ["omegafold", os.path.join(fpath, fasta_file), os.path.join(out_fpath)],
            capture_output=True,
        )
...
def clean_pdb(fpath, data_top_dir, pdb):
    ...
    subprocess.call(
        ["pdb_reres", os.path.join(fpath, pdb + "_clean.pdb")], stdout=reres_file
    )


def run_tmscore(
    fpath, pdb, num_seqs, path_to_tmscore="TMscore", amlt=False, reres=False
):
    """
    ...
    for i in range(num_seqs):
        ...
        if amlt:
            subprocess.call(
                [
                    "./" + path_to_tmscore,
                    os.path.join(out_fpath, pdb + "_reference.pdb"),
            ...
            print(
                "NOT USING RERES"
            )  # Manually switching between reference and re-res for multi-chain PDB TMscores - add more efficient approach
...
def wrap_dr_bert(
    ...
    assert os.path.exists(out_fpath), (
        "Can't find out path to generated_samples_string.fasta"
    )

    subprocess.call(
        [
            "python",
            path_to_dr_bert + "get_scores_fasta.py",
            path_to_dr_bert + "DR-BERT-final/",
            out_fpath + generated_fasta_file,
            out_fpath + out_file,
        ]
    )


def read_dr_bert_output(
    out_fpath,
    prefix,
    path_to_disorder_pickle,
    path_to_order_pickle,
    disorder_df,
    order_df,
):
    ...
    for i in range(len(disorder_df)):
        ...
        evodiff.plot.plot_idr_drbert_multiple(
            out_fpath + "/plots/",
            prefix + "_disorder_",
            drbert_disorder_out,
            s,
...
def wrap_disopred(
    fasta_file="gen_seq_0.fasta", path_to_disopred="BLAST+/run_disopred_plus.pl"
):
    """
...
def eval_disopred_output(out_fpath, ref_df, prefix="", num_seqs=100):
    ...
    for i in range(num_seqs):
        ...
        gen_df = pd.read_csv(
            ...
            comment="#",
            names=["resid", "residue", "marker", "score"],
        )
        og_df = pd.read_csv(
            og_file,
            delim_whitespace=True,
        ...
        evodiff.plot.plot_idr(
            out_fpath + "/plots/" + prefix + "gen_seq_", gen_df, s, e, i
        )
        evodiff.plot.plot_idr(
            out_fpath + "/plots/" + prefix + "true_seq_", og_df, s, e, i
        )
