...
from src.evodiff import plot
from src.evodiff.plot import plot_ecdf, plot_ecdf_bylength, plot_plddt_perp
...
def iterate_dirs_msa(run, mpnn=False):
    ...
    path = "/home/v-salamdari/data/DMs/amlt-generate-msa/" + str(run) + "esmif/"
    if mpnn:
        mpnn_path = "/home/v-salamdari/data/DMs/amlt-generate-msa/" + str(run) + "mpnn/"
    pdb_path = "/home/v-salamdari/data/DMs/amlt-generate-msa/" + str(run) + "pdb/"
    # Get ESMIF perp
...
def iterate_dirs(run, seq_lengths, mpnn=False):
    ...
    for l in seq_lengths:
        path = (
            "/home/v-salamdari/data/DMs/blobfuse/" + str(run) + "esmif/" + str(l) + "/"
        )
        if mpnn:
            mpnn_path = (
                "/home/v-salamdari/data/DMs/blobfuse/"
                + str(run)
                + "mpnn/"
                + str(l)
                + "/"
        ...
        pdb_path = (
            "/home/v-salamdari/data/DMs/blobfuse/" + str(run) + "pdb/" + str(l) + "/"
        )
        files = get_files(path)
        perplexity, perp_index = get_perp(files)
        perp_group.append(perplexity)
...
if length_model == "large":
    runs = [
        "test-data-2/",
        "d3pm/soar-640M/",
        "d3pm/oaardm-640M-backup/",
        "d3pm_uniform_640M/",
        "d3pm_blosum_640M/",  #'d3pm/random-640M-0/',  'd3pm/blosum-640M-0/',
        "hyper12/cnn-650M/",
        "esm-1b/",
        "esm2/",
        #'rfdiff/',#'foldingdiff/',
    ...
    runs = [
        "msa-oaardm-max-train-startmsa/valid/",
        "msa-oaardm-max-train-startmsa/gen/",
        "msa-oaardm-random-train-startmsa/gen/",
        "msa_oa_ar_maxsub_startrandomquery/gen/",
        "msa-esm-startmsa-t2/gen/",
        "potts/gen/",
    ]
...
for run in runs:
    ...
    if (
        run == "hyper12/cnn-650M/"
        or run == "esm-1b/"
        or run == "random-ref/"
        or run == "pretrain21/cnn-38M/"
    ):
    ...
    if sequences == True:
        if mpnn:
            (
                ...
                pdb_index_group,
                perp_index_group,
            ) = iterate_dirs(run, seq_lengths, mpnn=mpnn)
        else:
            (
                perp_group,
                scores_group,
        ...
        if mpnn:
            (
                ...
                pdb_index_group,
                perp_index_group,
            ) = iterate_dirs_msa(run, mpnn=mpnn)
        else:
            (
                perp_group,
                scores_group,
...
# For ESM-IF
print("ESM-IF")
evodiff.plot.plot_sc_boxplot(
    perp_groups, colors, labels, model="ESM-IF", length_model=length_model
)
    ...
    if sequences:
        ...
        for l_index in range(len(seq_lengths)):
            df_pdb = pd.DataFrame(
                np.array(
                    [
                        list(map(float, pdb_index_groups[i][l_index])),
                        list(map(float, scores_groups[i][l_index])),
                    ]
                ).T,
                columns=["pdb", "plddt"],
            )
            df_perp = pd.DataFrame(
                np.array(
                    [
                        list(map(float, perp_index_groups[i][l_index])),
                        list(map(float, perp_groups[i][l_index])),
                    ]
                ).T,
                columns=["pdb", "perp"],
            )
...
for idx in range(len(labels)):
    ...
    c_df = pd.DataFrame(
        np.array([ordered_plddt_group[idx], ordered_perp_group[idx]]).T,
        columns=["plddt", "perp"],
    )
    print(
        labels[idx],
