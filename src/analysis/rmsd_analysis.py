...
from evodiff.plot import (
    plot_conditional_rmsd,
    plot_conditional_sim,
    plot_conditional_tmscores,
)
...
def main():
    ...
    parser.add_argument("--scaffold-min", type=int, default=1, help="Min scaffold len ")
    parser.add_argument(
        "--scaffold-max",
        type=int,
        default=30,
    ...
    if not args.amlt:
        ...
        if not args.random_baseline:
            home += "/data/DMs/" + args.model_type + "/" + args.pdb
        else:
            home += "/data/DMs/random-baseline/" + args.pdb
    else:
    ...
    motif_df = calc_rmsd(
        ...
        ref_pdb,
        fpath=home,
        ref_motif_starts=args.start_idxs,
        ref_motif_ends=args.end_idxs,
    )
    ci_scores, ci_sampled, ci_fixed = get_confidence_score(
        home, args.num_seqs, motif_df
    )
    ...
    candidates = motif_df_sorted[
        (motif_df_sorted["rmsd"] <= 1) & (motif_df_sorted["scores"] >= 70)
    ]
    ...
    plot_conditional_tmscores(
        tm, ["grey"], legend=False, save_path=home + "plots/" + args.pdb
    )
...
def calc_rmsd(
    ...
    for i in range(
        ...
        for j in range(len(ref_motif_starts)):
            ref_selection += (
                str(ref_motif_starts[j] + 1) + ":" + str(ref_motif_ends[j] + 1) + " "
            )  # +1 (PDB indexed at 1)
