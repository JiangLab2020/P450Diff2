...
from evodiff.plot import (
    aa_reconstruction_parity_plot,
    msa_pairwise_interactions,
    msa_substitution_rate,
    plot_percent_similarity,
)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="?",
        default="/Users/nityathakkar/data/research/msr/model_output/",
    )
    ...
    parser.add_argument(
        "--start-query", action="store_true"
    )  # if starting from query -> gen msa
    parser.add_argument(
        "--start-msa", action="store_true"
    )  # if starting from msa, gen-> query
    args = parser.parse_args()

    project_dir = home + "/data/DMs/"
    try:
    ...
    if args.amlt:
        if args.start_valid:
            train_msas, gen_msas = preprocess_amlt_outputs(
                args.out_fpath,
                arg_mask=args.mask,
            ...
            tokenizer = sample_train_msa(
                os.path.join(data_top_dir, "openfold/"),
                arg_mask=args.mask,
                selection_type=args.subsampling,
                num_samples=50,
                out_path=project_dir + "ref/",
            )
            gen_msas = preprocess_amlt_outputs(
                args.out_fpath,
                arg_mask=args.mask,
                data_top_dir=data_top_dir,
                idr=args.idr,
            ...
            train_msas = np.load(
                ...
                + args.mask
                + args.subsampling
                + "_tokenized_openfold_train_msas.npy"
            )
    else:
        train_msas = np.load(
            project_dir
    ...
    msa_pairwise_interactions(
        gen_msas, train_msas, tokenizer.all_aas[:-7], args.out_fpath
    )


def preprocess_amlt_outputs(
    out_fpath, arg_mask, data_top_dir, idr=False, start_valid=False
):
    if arg_mask == "autoreg":
        tokenizer = Tokenizer()
    ...
    gen_files = [
        filename for filename in os.listdir(out_fpath) if filename.startswith("gen")
    ]
    print(gen_files)
    if start_valid:
            ...
            with open(valid_file, "r") as file:
                ...
                for row in filecontent:
                    if "SEQUENCE" in row[0] or "seq 0" in row[0]:
                        ...
                        if query_seq:
                            os.system(
                                "echo " + row[0] + " >> " + all_query_valid + ".txt"
                            )
                            all_query_valid_write_str += row[0] + "\n"
                        else:
                            os.system(
                                "echo " + row[0] + " >> " + all_msa_valid + ".txt"
                            )
                        tokenized_seq = tokenizer.tokenizeMSA(row[0])
                        valid_msa_arr[msa - 1, num_seqs, : len(tokenized_seq)] = (
                            tokenized_seq
                        )
                        num_seqs += 1
        with open(gen_file, "r") as file:
            ...
            for row in filecontent:
                if "SEQUENCE_" in row[0] or "seq 0" in row[0] or "MSA_0" in row[0]:
                    ...
                    gen_msa_arr[gen_msa - 1, gen_num_seqs, : len(tokenized_seq)] = (
                        tokenized_seq
                    )
                    gen_num_seqs += 1
                    if query_seq:
                        os.system(
                            "echo " + write_sequence + " >> " + all_query_gen + ".txt"
                        )
                        all_query_gen_write_str += write_sequence + "\n"
                        query_seq = False
                    else:
                        os.system(
                            "echo " + write_sequence + " >> " + all_msa_gen + ".txt"
                        )
                    write_sequence = ""
                else:
...
def preprocess_train_msa_file(
    out_fpath, f_name="valid_msas.a3m", arg_mask="autoreg", data_top_dir="data/"
):
    if arg_mask == "autoreg":
        tokenizer = Tokenizer()
    elif arg_mask == "blosum" or arg_mask == "random":
...
def sample_train_msa(
    ...
    dataset = A3MMSADataset(
        data_dir=train_msa_path,
        selection_type=selection_type,
        n_sequences=64,
        max_seq_len=256,
    )
    loader = DataLoader(
        dataset, batch_size=num_samples, collate_fn=collater, num_workers=8
    )
    ...
    np.save(
        out_path + arg_mask + selection_type + "_tokenized_openfold_train_msas",
        train_msas,
    )
