...
from evodiff.plot import plot_perp_group_d3pm, plot_perp_group_masked
    ...
    for i in tqdm(range(24000)):  # len(data))):
        ...
        if checkpoint[-1] == "causal-mask":
            ...
            if not np.isnan(loss):  # esm-1b predicts nans at large % mask
                ...
                if (
                    save_name == "esm1b_650M"
                    or save_name == "esm2_650M"
                    or save_name == "carp_640M"
                    or save_name == "carp_38M"
                ):
...
def sum_nll_mask(sequence, checkpoint):
    ...
    if scheme == "d3pm":
        ...
        ).float()  # placeholder, should be no pads since not batching
        mask = mask.cuda()
        input_mask = input_mask.cuda()
    elif scheme == "esm-mask":
        src, timestep, tgt, mask = collater(sequence)
        input_mask = (
            src != tokenizer.padding_idx
        ).float()  # placeholder, should be no pads since not batching
        mask = mask.cuda()
        input_mask = input_mask.cuda()
    ...
    return (
        ...
        nll_loss,
        tokens,
    )  # return timestep sampled (or % masked), sum of losses, and sum of tokens
